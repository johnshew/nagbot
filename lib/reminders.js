...
class RemindersMongo {
    ...
    get(id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let result = yield this.db.collection('reminders').findOne({ 'id': id });
    ...
    update(reminder) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.ready)
                yield this.initialized;
            let operation = yield this.db.collection('reminders').updateOne({ 'id': reminder.id }, { $set: reminder }, { upsert: true });
    ...
    delete(reminder) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.ready)
                yield this.initialized;
            let operation = yield this.db.collection('reminders').deleteOne({ 'id': reminder.id });
            if (operation.result.ok === 1)
                return;
            throw new Error('update failed');
        });
...
function close(callback) {
    let remindersInMemClose = exports.remindersInMem.close().then(() => console.log('remindersInMem closed'));
    let remindersMongoClose = exports.remindersMongo.close().then(() => console.log('remindersMongo closed'));
